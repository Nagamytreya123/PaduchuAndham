...
export async function completePaidOrder(
  ...
  const acquired = await acquirePaymentIdempotency(paymentId);
  if (!acquired) {
    const fresh = await OrderModel.findById(order._id.toString());
    if (fresh?.status === 'paid') {
      order.status = 'paid';
      order.razorpayPaymentId = fresh.razorpayPaymentId ?? paymentId;
    }
    return;
  }
  ...
  try {
    ...
    await enqueueOrderPaidEmails(payload);
  } catch (err) {
    await releasePaymentIdempotency(paymentId);
    throw err;
  }
}
